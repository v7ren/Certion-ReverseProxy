...
class WebSocketTunnelThread(threading.Thread):
    ...
    Now listens for 'command' messages and executes them.
    """
    def __init__(self, server_url, api_key, project_id, local_port, agent=None):
        ...
        while not self._stop_event.is_set():
            ...
            if msg.type in (aiohttp.WSMsgType.BINARY, aiohttp.WSMsgType.TEXT):
                ...
                if data.get("type") == "http_request":
                    ...
                    print(f"[Tunnel] Received command: {data}")
                    if self.agent:
                        self.agent.execute_command(data)
...
class ProjectAgent:
    ...
    def start_tunnel_thread(self, project_id, local_port):
        ...
        thread = WebSocketTunnelThread(self.server_url, self.api_key, project_id, local_port, agent=self)
