...
const projectsService = {
  ...
  async setPublic(id: number, isPublic: boolean): Promise<Project> {
...
const ProjectsView = ({ showToast }: ProjectsViewProps) => {
  ...
  const handleSetPublic = async () => {
    ...
    try {
      const updatedProject = await projectsService.setPublic(
        selectedProject.id,
      ...
      showToast(error.message || 'Failed to update project visibility', 'error');
    }
  };

  const handleStartProject = async (project: Project) => {
    console.log("START BUTTON CLICKED", project);
    try {
  ...
  const openPublicModal = (project: Project) => {
    setSelectedProject(project);
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-px bg-gray-200 mb-16">
        ...
        <div className="bg-white p-8 hover:bg-gray-50 transition-colors">
          ...
          <p className="text-sm font-light text-gray-500">Publicly visible</p>
        </div>
      </div>
              ...
              >
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        {project.is_public && (
                          <span className="text-xs px-2 py-1 bg-blue-100 text-blue-800 font-light tracking-widest uppercase">
                            PUBLIC
                          </span>
                        )}
                      </div>
                ...
                <div className="p-6">
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    ...
                    <button
                      onClick={() => openPublicModal(project)}
                      className={`flex items-center justify-center gap-2 px-3 py-2 border font-light text-sm tracking-wider transition-colors cursor-pointer ${
                        project.is_public
      ...
      {/* SET PUBLIC MODAL */}
      {showPublicModal && selectedProject && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 modal-overlay p-4">
          <div className="bg-white w-full max-w-md border border-gray-200 modal-content">
            <div className="border-b border-gray-200 p-6">
              <div className="flex items-center justify-between">
                <h3 className="text-2xl font-light tracking-wider">
                  {selectedProject.is_public ? 'Make Private' : 'Make Public'}
                </h3>
                <button 
                  onClick={() => {
                    setShowPublicModal(false);
            ...
            <div className="p-6">
              <div className="mb-6">
                {selectedProject.is_public ? (
                  <EyeOff className="w-12 h-12 text-gray-900 mb-4" />
                ) : (
                  <Eye className="w-12 h-12 text-gray-900 mb-4" />
                )}
                
                <p className="text-gray-700 font-light mb-2">
                  {selectedProject.is_public ? (
                    <>
                      Make <strong className="font-medium">{selectedProject.name}</strong> private?
                    </>
                  ) : (
                    <>
                      Make <strong className="font-medium">{selectedProject.name}</strong> public?
                    </>
                  )}
                </p>
                ...
                <p className="text-sm text-gray-500 font-light">
                  {selectedProject.is_public ? (
                    'This project will only be visible to you and authorized users.'
                  ) : (
                    'This project will be visible to anyone with the link. Make sure it doesn\'t contain sensitive information.'
                  )}
                </p>
              </div>
              ...
              <div className="mb-8">
                ...
                <div className="border border-gray-200 bg-white">
                  <div className="grid grid-cols-2 gap-px bg-gray-200">
                    ...
                    <div className="bg-white p-4">
                      <p className="text-xs font-light text-gray-500 tracking-widest uppercase mb-1">Visibility</p>
                      <p className="font-light text-sm">
                        {selectedProject.is_public ? (
